...
public class APIPublisherHome extends BasicTestObject {
	...
	public void i_should_see_apipublisher_username_at_the_top_right_corner_of_the_page(String arg1) throws Throwable {
		...
		Assert.assertTrue("Incorrect user login",
				publisherHome.isUserLogin(config.getValue(getEnvironment() + arg1 + "user")));
	}
	...
	public void i_should_see_the_apipublisher_page_label(String arg1) throws Throwable {
		...
		Assert.assertTrue("API Publisher label is not displayed",
				publisherHome.isPublisherLoginPageLabelDisplayed(arg1));
	}
	...
	public void i_validate_the_table_content_for_and(String arg1, String arg2, String arg3) throws Throwable {
		...
		Assert.assertTrue("The application nor the apis relevant to your search is listed in the table",
				publisherHome.isTableSubscriptions(username, arg2, arg3));
	}
	...
	public void i_search_API_with(String arg1) throws Throwable {
		...
		publisherHome.enterAPINameSearch(config.getValue(arg1));
		publisherHome.clickSearch();
		if(publisherHome.checkApiAvailability(config.getValue(arg1))) {
		// publisherHome.enterAPINameSearch(config.getValue(arg1));
		Thread.sleep(sleepTime/4);
	...
	@Then("^I should see the apipublisher error popup with \"([^\"]*)\" if \\\"([^\\\"]*)\\\" exits$")
	public void confirmApipublisher_errorPopupIfExists(String arg1, String arg2) throws Throwable {
		APIPublisherHomePage publisherHome = new APIPublisherHomePage(driver);
		if (publisherHome.checkApiAvailability(config.getValue(arg2))) {
			Assert.assertTrue(publisherHome.isApiPublisherError(arg1));
		}
	...
	@Then("^I click on apipublisher error popup ok button if \\\"([^\\\"]*)\\\" exits$")
	public void doClickOnApipublisher_error_popup_ok_buttonIfExists(String api) throws Throwable {
		APIPublisherHomePage publisherHome = new APIPublisherHomePage(driver);
		if (publisherHome.checkApiAvailability(config.getValue(api))) {
			ApplicationsPage apppage = new ApplicationsPage(driver);
			apppage.clickOk();
			Thread.sleep(sleepTime/4);
		}
