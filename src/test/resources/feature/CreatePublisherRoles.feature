...
@assignPermissionToAPIPublisherRolesOnDialog
  Scenario Outline:Hub administrator creates user role with API publishing permission
    Given I am in identity server page
    When I enter identity server username credentials
    And I click on identity server sign in
...
#    And I click on ids Add Role next button
  ...
  Examples:
      |role                |permissions |
      |pub-gitcc|API,Governance|  
      |pub-gitsoftware  |API,Governance|
      |pub-gitcrm  |API,Governance|
      |pub-gteckconp  |API,Governance|   
      |pub-conp  |API,Governance|
      |pub-GIT-Infra  |API,Governance|
      |pub-gitpsi  |API,Governance| 
